...
export function SchoolApplicationActions({ id, status }: { id: string; status: string }) {
  ...
  async function act(path: string) {
    setLoading(true);
    setError("");
    const res = await fetch(`${API_BASE}/api/family/school-applications/${id}${path}`, { method: "POST" });
        ...
        {status === "APPROVED" &&
          (confirming ? (
            <span className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-1.5 text-xs text-slate-600">
              I reviewed and authorize submission.
