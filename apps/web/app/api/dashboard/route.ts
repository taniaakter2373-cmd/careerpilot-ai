...
export async function GET() {
  ...
  const [jobsToday, strongMatches, pendingApproval, applied, interviews, offers, topMatches] = await Promise.all([
    ...
    prisma.job.findMany({ include: { matches: true }, orderBy: { scrapedAt: "desc" }, take: 10 }),
  ]);
  ...
  return NextResponse.json({ jobsToday, strongMatches, pendingApproval, applied, interviews, offers, topMatches: ranked });
}
